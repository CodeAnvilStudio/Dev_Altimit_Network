...
public class main {
    ...
    private static class Handler extends Thread {
        ...
        private byte[] GetMessage(byte[] _message, int _messageSize) {

            byte[] tCurrentMessage = null;
            //Lets make sure this message is long enough to even check for a key
            if(_messageSize <= _message.length) {

                //Get the last 4 bytes (should be the key)
                byte[] tMessageKey = Arrays.copyOfRange(_message, _messageSize - 4, _messageSize);

                //Make sure the full message is here
                if (Arrays.equals(tMessageKey, key)) {
                    tCurrentMessage = new byte[_messageSize - 8];
                    int tMessageOffset = 4;
                    System.arraycopy(_message, tMessageOffset, tCurrentMessage, 0, tCurrentMessage.length);
                }
            }
            ...
            try{
                while (true) {
                        ...
                        if (bufSize != 0 || fullMessage.length != 0) {
                            ...
                            System.out.println("Fullmessage is " + fullMessage.length + " long");
                            ...
                            if (messageSize == 0) {
                                ...
                                currentMessage = GetMessage(fullMessage, messageSize);
                                ...
                                if(currentMessage != null) {
                                    ...
                                    {
                                        System.out.println("Unable to convert message or no data in message.");
                                    }
                                    ...
                                    if (messageOffset <= fullMessage.length) {
                                        System.out.println("[Message Cleanup] Removing set amount.");
                                        fullMessage = Arrays.copyOfRange(fullMessage, messageOffset, fullMessage.length);
                                        messageOffset = 0;
                                        fullMessageSize = fullMessage.length;
                                    } else {
                                        System.out.println("[Message Cleanup] Data is larger, clearing!");
        ...
        public void InvokeMessage(List<Object> sentMessage){
            ...
            System.out.println(sentMessage.get(0).getClass());

            try {
                methodName = (String) sentMessage.get(0);
            }catch(ClassCastException e){
                System.out.println("Invoke Message Error: " + e);
            }
    ...
    public void finalize(){
        Set<UUID> clientUUIDS = localClientMap.keySet();
        for (UUID client : clientUUIDS) {
            DisconnectUser(client, true);
        }
